...
class livecounts:
    ...
    def __signature(timestamp: int) -> dict:
        return {
            ...
            'x-maven': sha256(
                f"0AVwElhWi1IfwcZKSNzq7E^84hFQ4ykenNAxeY7r@6ho1oTd6Ug*!WC&p$2aGY8MLHEkH0i8XCwnj3#JqI1NzCb91$gNzLYCbbG@NqvQMbcf8W9v3%s#uzjP@z*!e9a41JNWBqRIMJ*ULuav5k8z4kBj2^BCC%!3q@N0zZOS^TL#GzVz@9fhjg&^mSWi&oU5GMoCu9{timestamp}".encode()).hexdigest(),
...
class zefoy:
    ...
    def __title_loop(self):
        if name == 'nt':
            while True:
                ...
                system('title Mth Bot by @Jpno ^| Likes: %s Views: %s Shares: %s ^| %s ^| mode: hearts' % (
    ...
    def __ad_cookies(self) -> None:
        ...
        __ad_cookies = self.__session.get("https://partner.googleadservices.com/gampad/cookie.js",
                                          headers={'host': 'partner.googleadservices.com', 'connection': 'keep-alive',
                                                   'sec-ch-ua': '"Chromium";v="106", "Google Chrome";v="106", "Not;A=Brand";v="99"',
                                                   'sec-ch-ua-mobile': '?0',
                                                   'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36',
                                                   'sec-ch-ua-platform': '"Windows"', 'accept': '*/*',
                                                   'sec-fetch-site': 'cross-site', 'sec-fetch-mode': 'no-cors',
                                                   'sec-fetch-dest': 'script', 'accept-encoding': 'gzip, deflate, br',
                                                   'accept-language': 'en-US,en;q=0.9'},
                                          params={
                                              "domain": "zefoy.com",
                                              "callback": "_gfp_s_",
    ...
    def __search_link(self, __tiktok_link: str) -> None:
        try:
            ...
            self.__keys['key_2'], self.__aweme_id = findall(r'name="(.*)" value="(.*)" hidden', __search_link_response)[
                0]
            ...
            try:
                ...
                print('            ' + zefoy.sprint('*', 'sleeping -',
                                                    'for ' + Col.white + str(timer) + Col.blue + ' seconds'), end="\r")
                ...
                while time() < start + int(timer):
                    ...
                    print('            ' + zefoy.sprint('*', 'sleeping -',
                                                        'for ' + Col.white + time_left + Col.blue + ' seconds'),
                          end="\r");
                    sleep(1)
                print('            ' + zefoy.sprint('*', 'sending  -', 'hearts...                                  '),
                      end="\r")
                print('')
                self.__search_link(__tiktok_link)
    ...
    def mainloop(self) -> None:
        ...
        if self.__solve_captcha(__captcha_image) is True:
            print('            ' + zefoy.sprint('*', 'success  -',
                                                f'solved captcha: {Col.white}{round(time() - __start, 1)}{Col.blue}s'))
            video_link = input('            ' + zefoy.sprint('?', 'input    -', 'video link') + ' > ');
            print('\n')
            self.__item_id = livecounts.link_to_id(video_link)
            Thread(target=self.__title_loop).start()

            while True:
                self.__search_link(video_link);
                sleep(0.5)
                self.__send_req();
                sleep(1)
    ...
    @staticmethod
    def startup():
        system('cls' if name == 'nt' else '');
        system('title Like Bot by @Jpno ^| starting...')
        print(Col.purple + Center.XCenter(
            '''
                ▒█▀▄▀█ ▀▀█▀▀ █░░█ 　 ▒█▀▀█ █▀▀█ ▀▀█▀▀ 
                ▒█▒█▒█ ░░█░░ █▀▀█ 　 ▒█▀▀▄ █░░█ ░░█░░ 
                ▒█░░▒█ ░░▀░░ ▀░░▀ 　 ▒█▄▄█ ▀▀▀▀ ░░▀░░   \n\n                      made with <3 by Jpno   ''') + Col.reset);
        print("\n\n")
